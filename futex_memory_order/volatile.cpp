...
public:
    ...
    void lock() {
        ...
        while(!block.compare_exchange_strong(zero, thread_id, memory_order_relaxed)) {
    ...
    void unlock() {
        tid zero(0);
        tid thread_id = this_thread::get_id();
        if(!block.compare_exchange_strong(thread_id, zero, memory_order_relaxed)) {
            throw new exception;
        }
    }
...
int main() {
    freopen("res_volatile.txt", "w", stdout);
